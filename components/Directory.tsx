...
const cacheKey = "directoryDataV3_NFTPD"; // Unique cache key
const workspaceKey = "directoryWorkspaceState_NFTPD";
...
// Optimized Fetching using existing /api/explorer endpoint
async function batchFetchABIs(addresses: string[], existingCache: any) {
  ...
  for (let i = 0; i < toFetch.length; i += BATCH_SIZE) {
    ...
    await Promise.all(batch.map(async (address) => {
      try {
        // Use NFTPD's existing /api/explorer endpoint
        const response = await fetch(
          `/api/explorer/abi?address=${address}`
        );
        const data = await response.json();

        // NFTPD /api/explorer returns result as stringified JSON if status is "1"
        if (data.status === "1" && data.result) {
          try {
            const abi = JSON.parse(data.result);
            // We might need contract name too. /api/explorer/name?address=...
            // For now, default name or fetch separately? 
            // Let's try to fetch name or just use "Contract"
            const nameResp = await fetch(`/api/explorer/name?address=${address}`);
            const nameData = await nameResp.json();
            let name = "Contract";
            if (nameData.status === "1" && nameData.result?.[0]?.ContractName) {
              name = nameData.result[0].ContractName;
            }
...
const MethodPanel: React.FC<MethodPanelProps> = memo(({
  ...
  const monoBg = 'rgba(255,255,255,0.1)';
  ...
  return (
    <div
      ...
      style={{
        ...
        borderColor: isRead ? 'rgba(255, 255, 255, 0.3)' : 'rgba(200, 200, 200, 0.3)',
        boxShadow: executing ? `0 0 30px ${monoColor}` : '0 10px 30px rgba(0,0,0,0.5)',
      ...
      <div
        ...
        style={{
          backgroundImage: isRead
            ...
            : 'linear-gradient(90deg, rgba(200, 200, 200, 0.15), transparent)'
        }}
      >
        <div
        ...
        >
          {isRead ? <PlayCircleIcon className="w-5 h-5 text-white" /> : <BoltIcon className="w-5 h-5 text-gray-300" />}
      ...
      <div className="p-4 space-y-4 relative">
        ...
        <div className="absolute -left-1.5 top-16 flex flex-col gap-8 pointer-events-auto z-50">
          {inputs.map((_, i) => (
            <div
              ...
              className="w-3 h-3 rounded-full border border-black cursor-pointer hover:scale-150 transition-transform shadow-[0_0_10px_rgba(255,255,255,0.5)]"
            ...
            {inputs.map((input, idx) => (
              <div key={idx} className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  style={{
                    borderColor: isRead ? 'rgba(255,255,255,0.2)' : 'rgba(200,200,200,0.2)'
                  }}
                  onFocus={(e) => e.target.style.borderColor = isRead ? '#ffffff' : '#cccccc'}
                  onBlur={(e) => e.target.style.borderColor = isRead ? 'rgba(255,255,255,0.2)' : 'rgba(200,200,200,0.2)'}
                  placeholder="Value..."
                  value={input.value}
        ...
        <div className="pt-2 flex gap-2">
          <button
            ...
            style={{
              background: isRead ? 'rgba(255, 255, 255, 0.15)' : 'rgba(200, 200, 200, 0.15)',
              border: `1px solid ${isRead ? 'rgba(255, 255, 255, 0.4)' : 'rgba(200, 200, 200, 0.4)'}`,
              color: isRead ? '#ffffff' : '#cccccc',
            }}
          >
      ...
      {outputModalOpen && (
        <div className="absolute inset-0 bg-black/95 z-[60] flex flex-col p-4 animate-in fade-in zoom-in duration-200">
          <div className="flex justify-between items-center mb-2 pb-2 border-b border-white/10">
            <span className="text-xs font-mono text-white uppercase tracking-widest">Execution Result</span>
...
const Workspace: React.FC<WorkspaceProps> = ({ allABIs, diamondAddress }) => {
  ...
  const executeMethod = async (panelId: string) => {
    ...
    try {
      ...
      // Wait, passing fullABI of ALL facets to one contract instance?
      // Yes, Diamond pattern works this way if you treat the Diamond address as having ALL functions.
    ...
    >
      ...
      <div className="absolute top-4 right-4 z-[100] flex gap-2">
        <div className="flex bg-black/50 backdrop-blur-md border border-white/10 rounded-lg p-1">
          <button onClick={() => setScale(s => Math.min(s * 1.2, 3))} className="p-2 hover:bg-white/10 rounded text-white/70 hover:text-white"><MagnifyingGlassPlusIcon className="w-5 h-5" /></button>
          <button onClick={() => setScale(s => Math.max(s / 1.2, 0.2))} className="p-2 hover:bg-white/10 rounded text-white/70 hover:text-white"><MagnifyingGlassMinusIcon className="w-5 h-5" /></button>
          <div className="w-[1px] bg-white/10 mx-1" />
          <button onClick={() => { setScale(1); setPosition({ x: 0, y: 0 }); }} className="p-2 hover:bg-white/10 rounded text-white/70 hover:text-white"><ArrowPathIcon className="w-5 h-5" /></button>
      ...
      {panels.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-20">
          <div className="flex flex-col items-center">
            <CubeTransparentIcon className="w-24 h-24 text-white mb-4" />
      ...
      {clearModalOpen && (
        <div className="absolute inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#050a14] border border-white/30 rounded-lg p-6 max-w-sm w-full shadow-[0_0_30px_rgba(255,255,255,0.2)]">
            <h3 className="text-white font-mono font-bold text-lg mb-2 tracking-widest">CLEAR WORKSPACE</h3>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className="px-4 py-2 rounded bg-white/10 border border-white/50 text-white hover:bg-white/20 text-[10px] font-bold tracking-wider transition-all shadow-[0_0_20px_rgba(255,255,255,0.1)] hover:shadow-[0_0_30px_rgba(255,255,255,0.3)]"
              >
                CONFIRM_CLEAR
              </button>
            </div>
...
const MethodItem = ({ methodName, isRead, inputs, outputs }: any) => {
  ...
  return (
    <div ref={drag as unknown as React.LegacyRef<HTMLDivElement>} className={`group p-3 rounded-md border cursor-grab transition-all duration-200 relative overflow-hidden ${isDragging ? 'opacity-50' : 'opacity-100 hover:scale-[1.02]'
        ...
        ? 'bg-white/10 border-white/20 hover:border-white/60'
        : 'bg-gray-900/20 border-gray-500/20 hover:border-gray-400/60'
      }`}>
      <div className={`absolute left-0 top-0 bottom-0 w-1 ${isRead ? 'bg-white' : 'bg-gray-500'} opacity-50`} />
      <div className="flex justify-between items-center pl-2">
        <span className={`font-mono text-[11px] font-bold truncate ${isRead ? 'text-white' : 'text-gray-300'}`}>
          {methodName}
        </span>
      ...
      try {
        ...
        facetAddresses.forEach(addr => {
          ...
          if (fetched) {
            ...
            // NFTPD check logic - accept typical names or all
            // Ideally we check if it fits our protocol
            if (fetched.abi) {
    ...
    <DndProvider backend={HTML5Backend}>
      ...
      >
        ...
        <div className="flex-1 relative bg-[#050510] order-1 md:order-2">
          {/* pass diamondAddress to Workspace */}
          <Workspace allABIs={allABIs} diamondAddress={diamondAddress} />
        </div>

        {/* Sidebar / Bottom Bar */}
        <div className="w-full md:w-72 h-32 md:h-full flex-shrink-0 flex flex-col border-t md:border-t-0 md:border-r border-white/20 bg-[#050a14]/90 backdrop-blur-xl z-20 order-2 md:order-1">
          <div className="p-3 md:p-5 border-b border-white/20 bg-black/20 flex justify-between md:block items-center">
            <div>
              <h2 className="font-bold text-sm md:text-lg text-white tracking-[0.2em] mb-1">DIRECTORY</h2>
              <div className="text-[9px] md:text-[10px] text-white font-mono tracking-widest flex items-center gap-2">
          ...
          <div className="flex-1 overflow-x-auto md:overflow-y-auto overflow-y-hidden md:overflow-x-hidden p-2 md:p-3 space-x-4 md:space-x-0 md:space-y-6 custom-scrollbar flex flex-row md:flex-col items-center md:items-stretch">
            {loading ? (
              <div className="text-center py-4 md:py-20 space-x-4 md:space-x-0 md:space-y-4 flex md:block items-center justify-center w-full">
                <div className="w-6 h-6 md:w-10 md:h-10 border-2 border-white border-t-transparent rounded-full animate-spin mx-auto" />
                <div className="text-[10px] font-mono text-white animate-pulse tracking-widest whitespace-nowrap">
