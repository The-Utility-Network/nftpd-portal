...
function Spine({ length }: { length: number }) {
  return (
    <mesh rotation={[Math.PI / 2, 0, 0]} position={[0, -3.6, -length / 2]}>
      ...
      <meshStandardMaterial color={'#bfbfbf'} opacity={0.25} transparent />
    </mesh>
  );
}
...
export default function DiamondRings({ facets, methodNames, facetNames, facetAbis = {}, isMobile = false }: Props) {
  ...
  const rings = useMemo(() => {
    ...
    // Noir palette: generate grayscale shades instead of colored hues
    return facets.map((f, i) => {
      ...
      const lightness = 0.65 + ((i % 5) * 0.06); // 0.65..0.89
      const ringColor = new THREE.Color().setHSL(0, 0, Math.min(0.92, lightness)).getStyle();
  ...
  return (
    <div className="w-full h-full" style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'linear-gradient(180deg, rgba(7,7,7,0.65), rgba(20,20,20,0.55))', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)' }} />
      <Canvas gl={{ alpha: true }} style={{ background: 'transparent' }} camera={{ position: [0, isMobileView ? -0.06 : 0.0, isMobileView ? 3.95 : 2.6], fov: isMobileView ? 60 : 50 }} shadows>
        ...
        <directionalLight position={[-4, -2, -4]} intensity={0.5} color={'#e5e5e5'} />
      ...
      <div style={{ position: 'absolute', bottom: isMobileView ? 'calc(110px + env(safe-area-inset-bottom))' as unknown as number : 170, left: '50%', transform: 'translateX(-50%)', display: 'grid', gridTemplateColumns: 'auto 200px auto', alignItems: 'center', gap: 8, pointerEvents: 'auto', zIndex: 120 }}>
        {(() => {
          const hudColor = rings[currentIndex]?.baseColor || '#ffffff';
      ...
      {selection && (
        ...
        >
          <style>{`
            .diamond-card { scrollbar-width: thin; scrollbar-color: rgba(255,255,255,0.35) rgba(255,255,255,0.1); }
            .diamond-card::-webkit-scrollbar { width: 10px; height: 10px; }
            .diamond-card::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.35); border-radius: 10px; }
            .diamond-card::-webkit-scrollbar-track { background: rgba(255,255,255,0.08); }
            .diamond-card .micro-output { scrollbar-width: thin; scrollbar-color: rgba(255,255,255,0.35) rgba(255,255,255,0.1); }
            .diamond-card .micro-output::-webkit-scrollbar { width: 8px; height: 8px; }
            .diamond-card .micro-output::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.35); border-radius: 8px; }
          ...
          {selection.type === 'read' ? (
            <button
              disabled={busy}
              onClick={runRead}
              style={{ width: '100%', padding: '8px 10px', borderRadius: 6, background: selection.color, color: '#000', border: 'none', cursor: 'pointer', fontSize: 12, letterSpacing: '0.03em' }}
            >
              {busy ? 'Reading…' : 'Run Read'}
            </button>
          ) : (
            <button disabled={!walletAccount || busy} onClick={runWrite} style={{ width: '100%', padding: '8px 10px', borderRadius: 6, background: walletAccount ? selection.color : `${selection.color}66`, color: '#000', border: `1px solid ${selection.color}`, cursor: walletAccount ? 'pointer' : 'not-allowed', fontSize: 12, letterSpacing: '0.03em' }}>
